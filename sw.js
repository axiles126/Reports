...
const NOTES = [
  '3D: друга точка (ціль) — азимут і дальність рахуються від антени, висота цілі береться з рельєфу + висота над землею, показ прямої видимості',
  'Прогноз погоди береться за GPS (або точкою на мапі); оцінка «Гроза приблизно через N год» і джерело координат у підписі',
  '3D: гроза — темні хмари, спалахи й блискавки за прогнозом (WMO-код + CAPE), попередження про небезпеку польотів',
  '3D: вітер (напрям, швидкість, пориви) і опади за прогнозом, повзунок прогнозу до +47 год; дощ враховується в якості сигналу конуса',
